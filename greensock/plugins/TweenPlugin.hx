...
class TweenPlugin {
	...
	private function _addTween(target:Dynamic, propName:String, start:Float, end:Dynamic, overwriteProp:String = null, round:Bool = false):PropTween {
		var c:Float = ((end == null)) ? 0 : ((Compat.typeof((end)) == "number" || end.charAt(1) != "=")) ? Compat.parseFloat(end)
			- start : as3hx.Compat.parseInt(end.charAt(0) + "1") * as3hx.Compat.parseFloat(end.substr(2));
		if (c != 0) {
	...
	public function setRatio(v:Float):Void {
		...
		while (pt != null) {
			...
			if (pt.f) {
				Reflect.callMethod(pt.t, Reflect.field(pt.t, pt.p), [val]);
			} else {
				Reflect.setField(pt.t, pt.p, val);
			}
			pt = pt._next;
		}
	}
	...
	private static function _onTweenEvent(type:String, tween:TweenLite):Bool {
			...
			// sorts the PropTween linked list in order of priority because some plugins need to render earlier/later than others, like MotionBlurPlugin applies its effects after all x/y/alpha tweens have rendered on each frame.
		{
			...
			while (pt != null) {
				next = pt._next;
				pt2 = first;
				while (pt2 != null && pt2.pr > pt.pr) {
					pt2 = pt2._next;
				}
				if ((pt._prev = pt2 != null ? pt2._prev : last) != null) {
					pt._prev._next = pt;
				} else {
					first = pt;
				}
				if ((pt._next = pt2) != null) {
					pt2._prev = pt;
				} else {
					last = pt;
				}
		...
		while (pt != null) {
			if (pt.pg) {
				if (Reflect.hasField(pt.t, type)) {
					if (Reflect.callMethod(pt.t, Reflect.field(pt.t, type), [])) {
						changed = true;
					}
				}
			}
	...
	public static function activate(plugins:Array<Dynamic>):Bool {
		...
		while (--i > -1) {
			if (plugins[i].API == TweenPlugin.API) {
				// TweenLite._plugins[(new Type.getClass(plugins[i])()())._propName] = plugins[i];
				var plugin:Dynamic = Type.createInstance(plugins[i], []);
				TweenLite._plugins.set(Reflect.field(plugin, "_propName"), plugins[i]);
			}
		}
		return true;
	}
