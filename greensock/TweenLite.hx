...
class TweenLite extends Animation {
	...
	**/
	...
	private var _notifyPluginsOfEnabled:Bool = false;
	...
	public function new(target:Dynamic, duration:Float, vars:Dynamic) {
		...
		_overwrite = ((!(Lambda.has(this.vars,
			"overwrite")))) ? Reflect.field(_overwriteLookup,
				defaultOverwrite) : ((Compat.typeof((this.vars.overwrite)) == "number")) ? this.vars.overwrite >> 0 : Reflect.field(_overwriteLookup,
				Std.string(this.vars.overwrite));

		if (Std.isOfType(this.target, Array) && Compat.typeof((this.target[0])) == "object") {
				...
				// the next few lines accomplish the same thing as _siblings = _register(target, this, false) but faster and only slightly more verbose.
			{
				...
				// Reflect.setField(_tweenLookup, Std.string(target), [this]);
			} else {
	...
	private function _init():Void {
		...
		if (Std.isOfType(vars.ease, Ease)) {
			_ease = ((Std.isOfType(vars.easeParams, Array))) ? vars.ease.config.apply(vars.ease, vars.easeParams) : vars.ease;
		} else if (Compat.typeof((vars.ease)) == "function") {
		...
		if (_targets != null) {
			i = _targets.length;
			while (--i > -1) {
				if (_initProps(_targets[i], (_propLookup[i] = {}), _siblings[i],
					(_overwrittenProps != null) ? Reflect.field(_overwrittenProps, Std.string(i)) : null)) {
					initPlugins = true;
				}
			}
		} else {
		...
		if (_overwrittenProps != null) {
			if (_firstPT == null) {
				if (Compat.typeof((target)) != "function")
	...
	private function _initProps(target:Dynamic, propLookup:Dynamic, siblings:Array<Dynamic>, overwrittenProps:Dynamic):Bool {
		...
		for (p in Reflect.fields(vars)) {
			...
			if (Reflect.hasField(_reservedProps, p)) {
				if (Std.isOfType(val, Array)) {
					if (val.join("").indexOf("{self}") != -1) {
						Reflect.setField(vars, p, _swapSelfInParams(try cast(val, Array<Dynamic>) catch (e:Dynamic) null));
					}
				}
			} else if (Reflect.hasField(_plugins, p)) {
				var pluginClass:Class<Dynamic> = Reflect.field(_plugins, p);
				var plugin:Dynamic = Type.createInstance(pluginClass, []);
				if (plugin != null && plugin._onInitTween(target, val, this)) {
					...
					_firstPT = new PropTween(target, p, 0, 1, p, false, _firstPT);
					Reflect.setField(propLookup, p, _firstPT);
					// _firstPT.s = ((!_firstPT.f)) ? Compat.parseFloat(Reflect.field(target, p)) : Reflect.field(target, Std.string((((p.indexOf("set") || !(Lambda.has(target, "get" + p.substr(3))))) ? p : "get" + p.substr(3))))();
					_firstPT.s = (!_firstPT.f) ? Std.parseFloat(Reflect.field(target,
						p)) : (p.indexOf("set") != -1
							|| !Reflect.isObject(target)
							|| !Reflect.hasField(target,
								"get" + p.substr(3))) ? Reflect.field(target, p) : Reflect.callMethod(target, Reflect.field(target, "get" + p.substr(3)), []);
					// _firstPT.c = ((Compat.typeof((val)) == "number")) ? Compat.parseFloat(val) - _firstPT.s : ((Compat.typeof((val)) == "string" && val.charAt(1) == "=")) ? Compat.parseInt(val.charAt(0) + "1") * Compat.parseFloat(val.substr(2)) : Compat.parseFloat(val) || 0;
					_firstPT.c = if (Std.isOfType(val, Float)) {
						Std.parseFloat(val) - _firstPT.s;
					} else if (Std.isOfType(val, String) && val.charAt(1) == '=') {
						Std.parseInt(val.charAt(0) + '1') * Std.parseFloat(val.substr(2));
					} else {
						Std.parseFloat(val);
					};
				}
	...
	override public function render(time:Float, suppressEvents:Bool = false, force:Bool = false):Void {
		...
		if (time >= _duration) {
				...
				// zero-duration tweens are tricky because we must discern the momentum/direction of time in order to determine whether the starting values should be rendered or the ending values. If the "playhead" of its timeline goes past the zero-duration tween in the forward direction or lands directly on it, the end values should be rendered, but if the timeline's "playhead" moves past it in the backward direction (from a postitive time to a negative time), the starting values must be rendered.
			{
				...
				if (time == 0 || rawPrevTime < 0 || rawPrevTime == Animation._tinyNum) {
					if (rawPrevTime != time) {
						force = true;
						if (rawPrevTime > 0 && rawPrevTime != Animation._tinyNum) {
			...
			// to work around occasional floating point math artifacts, round super small values to 0.
		{
			...
			if (prevTime != 0 || (_duration == 0 && _rawPrevTime > 0 && _rawPrevTime != Animation._tinyNum)) {
				callback = "onReverseComplete";
		...
		while (pt != null) {
			if (pt.f) {
				Reflect.callMethod(pt.t, Reflect.field(pt.t, pt.p), [pt.c * ratio + pt.s]);
			} else {
		...
		if (_onUpdate != null) {
			...
			if (!suppressEvents) {
				if (_time != prevTime || isComplete) {
					Reflect.callMethod(null, _onUpdate, vars.onUpdateParams);
				}
			}
		}
	...
	override public function _kill(vars:Dynamic = null, target:Dynamic = null):Bool {
		...
		if (target != null) {
			// Do nothing, target already has a value
		...
		if (Std.isOfType(target, Array) && Compat.typeof((Reflect.field(target, Std.string(0)))) == "object") {
			i = target.length;
			while (--i > -1) {
			...
			if (_targets != null) {
				...
				while (--i > -1) {
					if (target == _targets[i]) {
						propLookup = _propLookup[i] != null ? _propLookup[i] : {};
						if (_overwrittenProps == null)
							_overwrittenProps = [];
						overwrittenProps = _overwrittenProps[i] = vars ? (_overwrittenProps[i] != null ? _overwrittenProps[i] : {}) : "all";
						break;
					}
				}
				...
				overwrittenProps = _overwrittenProps = vars != null ? ((_overwrittenProps != null) ? _overwrittenProps : {}) : "all";
			}
			if (propLookup != null) {
				...
				record = (vars != overwrittenProps
					...
					&& vars != propLookup
					&& (Compat.typeof((vars)) != "object" || vars._tempKill != true)); // _tempKill is a super-secret way to delete a particular tweening property but NOT have it remembered as an official overwritten property (like in BezierPlugin)
	...
	public static function _prepVars(vars:Dynamic, immediateRender:Bool = false):Dynamic {
		if (vars._isGSVars) {
			vars = vars.vars;
		}
	 ...
	 */
	public static function delayedCall(delay:Float, callback:Function, params:Array<Dynamic> = null, useFrames:Bool = false):TweenLite {
	...
	private static function _dumpGarbage(event:Event):Void {
		if (Std.int(Animation._rootFrame / 60) == Animation._rootFrame / 60)
			// faster than !(_rootFrame % 60)
		{
			...
			for (tgt in Reflect.fields(_tweenLookup)) {
				...
				if (a.length == 0) {
					...
					Reflect.deleteField(_tweenLookup, tgt);
				}
			}
		}
	}
	...
	**/
	public static function killTweensOf(target:Dynamic, onlyActive:Dynamic = false, vars:Dynamic = null):Void {
		if (Compat.typeof((onlyActive)) == "object") {
	...
	**/
	public static function getTweensOf(target:Dynamic, onlyActive:Bool = false):Array<Dynamic> {
		...
		if (Std.isOfType(target, Array)
			&& Compat.typeof((Reflect.field(target, Std.string(0)))) != "string"
			&& Compat.typeof((Reflect.field(target, Std.string(0)))) != "number") {
			i = target.length;
			a = [];
			while (--i > -1) {
	...
	**/
	...
	private static function _applyOverwrite(target:Dynamic, tween:TweenLite, props:Dynamic, mode:Int, siblings:Array<Dynamic>):Bool {
		...
		var globalStart:Float = 0.0;
		i = siblings.length;
		while (--i > -1) {
			curTween = siblings[i];
			if (curTween == tween || curTween._gc || curTween._paused) { // ignore
	...
	**/
	private static function _checkOverlap(tween:Animation, reference:Float, zeroDur:Bool):Float {
		...
		t /= ts;
		return ((t > reference)) ? t - reference : (((zeroDur && t == reference)
			|| (!tween._initted && t - reference < 2 * min))) ? min : (((t += tween.totalDuration() / tween._timeScale / ts) > reference + min)) ? 0 : t
				- reference
				- min;
	}
}
