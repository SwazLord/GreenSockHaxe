...
class TweenLiteVars
{
    ...
    public function new(vars : Dynamic = null)
    {
        ...
        if (vars != null)
        {
            ...
            {
                Reflect.setField(_vars, p, Reflect.field(vars, p));
            }
        }
    }
    ...
    /**
		 ...
		 */
    public function bevelFilter(distance : Float = 4, angle : Float = 45, highlightColor : Int = 0xFFFFFF, highlightAlpha : Float = 0.5, shadowColor : Int = 0x000000, shadowAlpha : Float = 0.5, blurX : Float = 4, blurY : Float = 4, strength : Float = 1, quality : Int = 2, remove : Bool = false, addFilter : Bool = false, index : Int = -1) : TweenLiteVars
    {
        var filter : Dynamic = {
            distance : distance,
            angle : angle,
...
TweenLite.to(mc, 1, new TweenLiteVars().colorMatrixFilter(0xFF0000));
		 ...
		 * @return The TweenLiteVars instance
		 */
    public function colorMatrixFilter(colorize : Int = 0xFFFFFF, amount : Float = 1, saturation : Float = 1, contrast : Float = 1, brightness : Float = 1, hue : Float = 0, threshold : Float = -1, remove : Bool = false, addFilter : Bool = false, index : Int = -1) : TweenLiteVars
    {
        var filter : Dynamic = {
    ...
    /**
		 ...
		 * @return The TweenLiteVars instance
		 */
    public function colorTransform(tint : Float, tintAmount : Float, exposure : Float, brightness : Float, redMultiplier : Float, greenMultiplier : Float , blueMultiplier : Float , alphaMultiplier : Float , redOffset : Float , greenOffset : Float , blueOffset : Float , alphaOffset : Float ) : TweenLiteVars
    {
		if(tint == null){
			tint = Math.NaN;
		}
		...
		if(redMultiplier == null){
			redMultiplier = Math.NaN;
		}
		if(greenMultiplier == null){
			greenMultiplier = Math.NaN;
		}
		if(blueMultiplier == null){
			blueMultiplier = Math.NaN;
		}
		if(alphaMultiplier == null){
			alphaMultiplier = Math.NaN;
		}
		if(redOffset == null){
			redOffset = Math.NaN;
		}
		if(greenOffset == null){
			greenOffset = Math.NaN;
		}
		if(blueOffset == null){
			blueOffset = Math.NaN;
		}
		if(alphaOffset == null){
			alphaOffset = Math.NaN;
		}
        ...
        {
            if (Math.isNaN(Reflect.field(values, p)))
            {
                Reflect.deleteField(values, p);
            }
        }
    ...
    /**
		 ...
		 * @return The TweenLiteVars instance
		 */
    public function dropShadowFilter(distance : Float = 4, blurX : Float = 4, blurY : Float = 4, alpha : Float = 1, angle : Float = 45, color : Int = 0x000000, strength : Float = 2, inner : Bool = false, knockout : Bool = false, hideObject : Bool = false, quality : Int = 2, remove : Bool = false, addFilter : Bool = false, index : Int = -1) : TweenLiteVars
    {
        var filter : Dynamic = {
            distance : distance,
            blurX : blurX,
...
TweenLite.to(mc, 1, new TweenLiteVars().frame(125)); 
    ...
    /**
		 ...
		 * @return The TweenLiteVars instance
		 */
    public function glowFilter(blurX : Float = 10, blurY : Float = 10, color : Int = 0xFFFFFF, alpha : Float = 1, strength : Float = 2, inner : Bool = false, knockout : Bool = false, quality : Int = 2, remove : Bool = false, addFilter : Bool = false, index : Int = -1) : TweenLiteVars
    {
        var filter : Dynamic = {
            blurX : blurX,
            blurY : blurY,
...
TweenLite.to(myComponent, 1, new TweenLiteVars().setSize(200, 30)); 
		 ...
		 **/
    public function setSize(?width : Float, ?height : Float) : TweenLiteVars
    {
		if(width == null){
			width = Math.NaN;
		}
