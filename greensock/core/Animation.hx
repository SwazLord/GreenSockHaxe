...
Animation.ticker.removeEventListener("tick", myFunction);
    ...
    /** @private **/
    public static var _tinyNum : Float = 0.0000000001;
    ...
    public function new(duration : Float = 0, vars : Dynamic = null)
    {
        ...
        _duration = _totalDuration = (duration != null && duration != 0) ? duration : 0;
        _delay = this.vars.delay != null ? Std.parseFloat(Std.string(this.vars.delay)) : 0;
        _timeScale = 1;
        _totalTime = _time = 0;
        data = this.vars.data;
        _rawPrevTime = -1;
...
myAnimation.play(2, false);
 ...
 myAnimation.seek(2, false);
    ...
    {
        return totalTime(Compat.parseFloat(time), suppressEvents);
    }
...
myAnimation.eventCallback("onComplete", myFunction); //sets the onComplete
 ...
 myAnimation.totalDuration(2); //sets total duration
    ...
    {
		if (value == null) value = Math.NaN;
        _dirty = false;
        return ((Math.isNaN(value))) ? _totalDuration : duration(value);
    }
...
myAnimation.time(2); //sets time, jumping to new value just like seek().
		 ...
		 * @see #totalTime()
		 **/
    public function time(?value : Float, suppressEvents : Bool = false) : Dynamic
    {
		if (value == null)
        {
            return _time;
 ...
 myAnimation.totalTime(2); //sets total time, jumping to new value just like seek().
		 ...
		 * @see #pause()
		 **/
    public function totalTime(?time : Float, suppressEvents : Bool = false, uncapped : Bool = false) : Dynamic
    {
		if (time == null) time = Math.NaN;
        if (Math.isNaN(time) && suppressEvents == null && uncapped == null)
        {
            return _totalTime;
        }
        if (_timeline != null)
...
myAnimation.progress(0.25); //sets progress to one quarter finished
		 ...
		 * @see #totalProgress()
		 **/
    public function progress(?value : Float, suppressEvents : Bool = false) : Dynamic
    {
		if (value == null) value = Math.NaN;
        return (Math.isNaN(value) && suppressEvents == null) ? _time / duration() : totalTime(duration() * value, suppressEvents);
    }
...
myAnimation.totalProgress(0.25); //sets total progress to one quarter finished
		 ...
		 * @see #totalTime()
		 **/
    public function totalProgress(?value : Float, suppressEvents : Bool = false) : Dynamic
    {
		if (value == null) value = Math.NaN;
        return (Math.isNaN(value) && suppressEvents == null) ? _time / duration() : totalTime(duration() * value, suppressEvents);
    }
...
myAnimation.timeScale( 0.5 ); //sets timeScale to half-speed
    ...
    public function timeScale(?value : Float) : Dynamic
    {
        ...
        value = (value != null && value != 0) ? value : 0.000001; //can't allow zero because it'll throw the math off  
