...
class VideoLoader extends LoaderItem {
	...
	/** @private **/
	private var _stageVideo:Dynamic; // don't type as StageVideo because that would break publishing to FP9

	/** @private **/
	private var _sound:SoundTransform;

	/** @private **/
	private var _videoPaused:Bool = false;

	/** @private **/
	private var _videoComplete:Bool = false;
	...
	/** @private **/
	private var _pausePending:Bool = false;
	...
	/** @private **/
	private var _bufferMode:Bool = false;
	...
	private var _renderedOnce:Bool = false;
	...
	/** @private **/
	private var _autoDetachNetStream:Bool = false;
	...
	private var _playStarted:Bool = false;
	...
	private var _finalFrame:Bool = false;
	...
	public var autoAdjustBuffer:Bool = false;
	...
	public function new(urlOrRequest:Dynamic, vars:Dynamic = null) {
		...
		_duration = (Math.isNaN(this.vars.estimatedDuration)) ? 200 : as3hx.Compat.parseFloat(this.vars.estimatedDuration); // just set it to a high number so that the progress starts out low.
		...
		this.volume = ((Lambda.has(this.vars, "volume"))) ? as3hx.Compat.parseFloat(this.vars.volume) : 1;
	...
	private function _refreshNetStream():Void {
		...
		_ns = ((Std.isOfType(this.vars.netStream, NetStream))) ? this.vars.netStream : new NetStream(_nc);
		...
		_ns.bufferTime = (Math.isNaN(this.vars.bufferTime)) ? 5 : as3hx.Compat.parseFloat(this.vars.bufferTime);
	...
	override private function _dump(scrubLevel:Int = 0, newStatus:Int = 0, suppressEvents:Bool = false):Void {
		...
		if (scrubLevel >= 2) {
			...
			(try cast(_sprite, Dynamic) catch (e:Dynamic) null)
				.gcProtect = ((scrubLevel == 3)) ? null : _ns; // we need to reference the NetStream in the ContentDisplay before forcing garbage collection, otherwise gc kills the NetStream even if it's attached to the Video and is playing on the stage!
			_ns.client = {};
			_video = null;
			_ns = null;
			_nc = null;
	...
	override private function _calculateProgress():Void {
		...
		if (_cachedBytesLoaded > 1) {
			...
			if (_cachedBytesTotal <= _cachedBytesLoaded) {
				_cachedBytesTotal = (((this.metaData != null && _renderedOnce && _initted)
					|| (Math.round(haxe.Timer.stamp() * 1000) - _time >= 10000))) ? _cachedBytesLoaded : as3hx.Compat.parseInt(1.01 * _cachedBytesLoaded)
					+ 1;
			}
			if (!_auditedSize) {
				_auditedSize = true;
	...
	public function getCuePointTime(name:String):Float {
		if (this.metaData != null && Std.isOfType(this.metaData.cuePoints, Array)) {
	...
	**/
	...
	private function _forceInit():Void {
		...
		cast((_sprite), Object).rawContent = _video; // resizes it appropriately
	...
	private function _metaDataHandler(info:Dynamic):Void {
		...
		if (this.metaData.cuePoints) {
			var a:Array<Dynamic> = this.metaData.cuePoints;
			var i:Int = a.length;
			while (--i > -1) {
	...
	private function _playProgressHandler(event:Event):Void {
		if (!_bufferFull
			&& !_videoComplete
			&& (_ns.bufferLength >= _ns.bufferTime || this.duration - this.videoTime - _ns.bufferLength < 0.1))
		...
		if (_bufferFull && (_firstCuePoint != null || _dispatchPlayProgress)) {
			...
			var prevCueTime:Float = _prevCueTime;
			_prevTime = _prevCueTime = ((((_forceTime != 0 && !Math.isNaN(_forceTime)) || _forceTime == 0)
				&& _ns.time <= _duration)) ? _ns.time : this.videoTime; // note: isNaN(_forceTime) is much slower than !(_forceTime || _forceTime == 0)
			var next:CuePoint;
			var cp:CuePoint = _firstCuePoint;
			while (cp) {
				next = cp.next;
	...
	**/
	override public function auditSize():Void {
			...
			// if the url isn't relative, use the regular URLStream to do the audit because it's faster/more efficient.
		{
			...
			_auditNS.bufferTime = (Math.isNaN(this.vars.bufferTime)) ? 5 : as3hx.Compat.parseFloat(this.vars.bufferTime);
			_auditNS.client = {
	...
	private function _auditHandler(event:Event = null):Void {
		...
		var code:String = ((event == null || !(Std.isOfType(event, NetStatusEvent)))) ? "" : cast((event), NetStatusEvent).info.code;
	...
	override private function _auditStreamHandler(event:Event):Void {
		if (Std.isOfType(event, ProgressEvent) && _bufferMode) {
	...
	private function get_playProgress():Float // Often times the duration MetaData that gets passed in doesn't exactly reflect the duration, so after the FLV is finished playing, the time and duration wouldn't equal each other, so we'd get percentPlayed values of 99.26978. We have to use this _videoComplete variable to accurately reflect the status.
	{
