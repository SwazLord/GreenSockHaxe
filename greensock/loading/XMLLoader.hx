...
@:meta(Event(name = "init", type = "com.greensock.events.LoaderEvent"))
...
class XMLLoader extends DataLoader {
			...
			if (add && this.vars.integrateProgress != false) {
				for (p in Reflect.fields(_listenerTypes)) {
					if (p != "onProgress" && p != "onInit") {
						_loadingQueue.addEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent, false, -100, true);
					}
				}
				...
				for (p in Reflect.fields(_listenerTypes)) {
					if (p != "onProgress" && p != "onInit") {
						_loadingQueue.removeEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent);
					}
				}
			}
		}
	...
	override private function _calculateProgress():Void {
		...
		var estimate:Int = as3hx.Compat.parseInt(this.vars.estimatedBytes);
		if (this.vars.integrateProgress == false) { // do nothing
		} else if (_loadingQueue != null
			&& (as3hx.Compat.parseInt(this.vars.estimatedBytes) < _cachedBytesLoaded || _loadingQueue.auditedSize))
		...
		} else if (as3hx.Compat.parseInt(this.vars.estimatedBytes) > _cachedBytesLoaded
			&& (!_initted || (_loadingQueue != null && _loadingQueue.status <= LoaderStatus.COMPLETED && !_loadingQueue.auditedSize))) {
	...
	/** @private **/
	private static function _parseVars(xml:FastXML):Dynamic {
		var v:Dynamic = {
			rawXML: xml
		};
		var s:String;
		...
		var list:FastXMLList = xml.node.attributes.innerData();
		for (attribute in list) {
			s = attribute.node.name.innerData();
			...
			type = as3hx.Compat.typeof((Reflect.field(_varTypes, s)));
			if (type == "boolean") {
				Reflect.setField(v, s, cast(value == "true" || value == "1", Bool));
			} else if (type == "number") {
				Reflect.setField(v, s, as3hx.Compat.parseFloat(value));
			} else {
				Reflect.setField(v, s, value);
			}
		}
		return v;
	}
	...
	public static function parseLoaders(xml:FastXML, all:LoaderMax, toLoad:LoaderMax = null):Void {
		var node:FastXML;
		var nodeName:String = Std.string(xml.node.name.innerData()).toLowerCase();
		if (nodeName == "loadermax") {
			var queue:LoaderMax = try cast(all.append(new LoaderMax(_parseVars(xml))), LoaderMax) catch (e:Dynamic) null;
			if (toLoad != null && queue.vars.load) {
				toLoad.append(queue);
			}

			if (queue.vars.childrenVars != null && queue.vars.childrenVars.indexOf(":") != -1) {
				queue.vars.childrenVars = _parseVars(new FastXML("<childrenVars "
					+ queue.vars.childrenVars.split(",").join("\" ").split(":").join("=\"")
					+ "\" />"));
			}
			...
			if (Lambda.has(queue.vars, "replaceURLText")) {
				var replaceText:Array<Dynamic> = queue.vars.replaceURLText.split(",");
				var i:Int = 0;
			...
			if (Lambda.has(queue.vars, "prependURLs")) {
				queue.prependURLs(queue.vars.prependURLs, false);
			}
		} else {
			if (Lambda.has(_types, nodeName)) {
				var loaderClass:Class<Dynamic> = Reflect.field(_types, nodeName);
				var parsedVars:Dynamic = _parseVars(xml);
				if (as3hx.Compat.typeof((all.vars.childrenVars)) == "object") {
					for (p in Reflect.fields(all.vars.childrenVars)) {
						if (!(Lambda.has(parsedVars, p))) {
							Reflect.setField(parsedVars, p, all.vars.childrenVars[p]);
						}
					}
				}
				var loader:LoaderCore = all.append(Type.createInstance(loaderClass, [xml.att.url, parsedVars]));
	...
	override private function _receiveDataHandler(event:Event):Void {
		try {
			_content = new FastXML(_loader.data);
		} catch (error:Error) {
		...
		_loadingQueue = new LoaderMax({
			...
			maxConnections: (as3hx.Compat.parseInt(this.vars.maxConnections) || 2),
			skipFailed: cast(this.vars.skipFailed != false, Bool),
			skipPaused: cast(this.vars.skipPaused != false, Bool)
		});
		_parsed = new LoaderMax({
			name: this.name + "_ParsedLoaders",
			paused: true
		});
		parseLoaders(try cast(_content, FastXML) catch (e:Dynamic) null, _parsed, _loadingQueue);
		if (_parsed.numChildren == 0) {
			_parsed.dispose(false);
			_parsed = null;
		} else if (Lambda.has(this.vars, "recursivePrependURLs")) {
			_parsed.prependURLs(this.vars.recursivePrependURLs, true);
			var loaders:Array<Dynamic> = _parsed.getChildren(true, true);
			var i:Int = loaders.length;
			while (--i > -1) {
				if (Std.isOfType(loaders[i], XMLLoader)) {
					loaders[i].vars.recursivePrependURLs = this.vars.recursivePrependURLs;
				}
			}
		} else if (Lambda.has(this.vars, "prependURLs")) {
