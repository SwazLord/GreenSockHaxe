...
class ImageLoader extends DisplayObjectLoader {
	...
	override private function _load():Void {
		...
		{
			var loaders:Array<Dynamic> = _globalRootLoader.getChildren(true, true);
			...
			while (--i > -1) {
				...
				if (loader.url == _url
					...
					&& Std.isOfType(loader, ImageLoader)
					&& Std.isOfType(cast((loader), ImageLoader).rawContent, Bitmap)) {
					_closeStream();
