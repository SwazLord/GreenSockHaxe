...
class SWFLoader extends DisplayObjectLoader {
			...
			if (add && this.vars.integrateProgress != false) {
				for (p in Reflect.fields(_listenerTypes)) {
					if (p != "onProgress" && p != "onInit") {
						_queue.addEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent, false, -100, true);
					}
				}
				...
				for (p in Reflect.fields(_listenerTypes)) {
					if (p != "onProgress" && p != "onInit") {
						_queue.removeEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent);
					}
				}
			}
		}
	...
	/** @private **/
	override private function _determineScriptAccess():Void // don't test the BitmapData.draw() until the swf has fully loaded because it can incorrectly throw security errors in certain situations (like NetStreams that haven't started yet).
	{
		try {
	...
	override private function _calculateProgress():Void {
		...
		} else if (_queue != null && (as3hx.Compat.parseInt(this.vars.estimatedBytes) < _cachedBytesLoaded || _queue.auditedSize))
	...
	public function getClass(className:String):Class<Dynamic> {
		...
		if (_content.loaderInfo.applicationDomain.hasDefinition(className)) {
			...
			var result:Dynamic;
			var loaders:Array<Dynamic> = _queue.getChildren(true, true);
	...
	/** @private **/
	override private function _initHandler(event:Event):Void // if the SWFLoader was cancelled before _initHandler() was called, Flash will refuse to properly unload it, so we allow it to continue but check the status here and _dump() if necessary.
	{
		if (_stealthMode) {
			_initted = true;
		...
		try {
			...
			var className:String = Type.getClassName(tempContent);
			if (className.substr(-13) == "__Preloader__") {
				var rslPreloader:Dynamic = Reflect.field(tempContent, "__rslPreloader");
	...
	private function _rslAddedHandler(event:Event):Void // check to ensure this was actually something added to the _loader.content
	{
		if (Std.isOfType(event.target, DisplayObject)
			&& Std.isOfType(event.currentTarget, DisplayObjectContainer)
			&& event.target.parent == event.currentTarget) {
			_rslAddedCount++;
		}
