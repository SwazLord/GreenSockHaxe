...
class ContentDisplay extends Sprite {
	...
	/** @private **/
	private var _centerRegistration:Bool = false;
	...
	private function _update():Void {
			...
			// Video objects don't accurately report getBounds() - they act like their native dimension is always 160x320.
		{
			...
			if (Std.isOfType(nativeBounds, LoaderInfo) && _loader != null && _loader.progress < 1) {
				try {
		...
		if (_fitWidth > 0 && _fitHeight > 0) {
			...
			if (_scaleMode != "none") {
				...
				if ((contentRatio < displayRatio && _scaleMode == "proportionalInside")
					|| (contentRatio > displayRatio && _scaleMode == "proportionalOutside")) {
					w = h * contentRatio;
				}
				if ((contentRatio > displayRatio && _scaleMode == "proportionalInside")
					|| (contentRatio < displayRatio && _scaleMode == "proportionalOutside")) {
	...
	**/
	...
	private function set_loader(value:LoaderItem):LoaderItem {
		...
		for (p in Reflect.fields(_transformProps)) {
			if (Lambda.has(_loader.vars, p)) {
				...
				Reflect.setField(this, p,
					((type == "number")) ? as3hx.Compat.parseFloat(_loader.vars[p]) : ((type == "string")) ? Std.string(_loader.vars[p]) : cast(_loader.vars[p],
						Bool));
			}
		}
		...
		_update();
		if (Std.isOfType(_loader.vars.container, DisplayObjectContainer)) {
