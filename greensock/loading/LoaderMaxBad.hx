...
class LoaderMax extends LoaderCore {
	...
	public function new(vars:Dynamic = null) {
		...
		this.maxConnections = ((Lambda.has(this.vars, "maxConnections"))) ? as3hx.Compat.parseInt(this.vars.maxConnections) : 2;
		if (Std.isOfType(this.vars.loaders, Array)) {
	...
	public static function parse(data:Dynamic, vars:Dynamic = null, childrenVars:Dynamic = null):Dynamic {
		if (Std.isOfType(data, Array)) {
			...
			return queue;
		} else if (Std.isOfType(data, String) || Std.isOfType(data, URLRequest)) {
			var s:String = ((Std.isOfType(data, String))) ? data : cast((data), URLRequest).url;
	...
	public function insert(loader:LoaderCore, index:Int = 999999999):LoaderCore {
		...
		if (this != _globalRootLoader) {
			for (p in Reflect.fields(_listenerTypes)) {
				if (p != "onProgress" && p != "onInit") {
					loader.addEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent, false, -100, true);
				}
			}
			loader.addEventListener(LoaderEvent.PROGRESS, _progressHandler, false, -100,
				true); // use -1 so that if the user adds an event listener, it gets called before LoaderMax is notified. Otherwise bubbling behavior doesn't go in the proper order.
		...
		_cacheIsDirty = true;
		if (_status == LoaderStatus.LOADING) { // do nothing
	...
	override private function _dump(scrubLevel:Int = 0, newStatus:Int = 0, suppressEvents:Bool = false):Void {
		if (newStatus == LoaderStatus.DISPOSED) {
			...
			if (Std.isOfType(this.vars.requireWithRoot, DisplayObject)) {
	...
	override private function _calculateProgress():Void {
		...
		while (--i > -1) {
			loader = _loaders[i];
			s = loader.status;
			if (s <= LoaderStatus.COMPLETED
				|| (!this.skipPaused && s == LoaderStatus.PAUSED)
				|| (!this.skipFailed && s == LoaderStatus.FAILED)) {
	...
	private function _removeLoaderListeners(loader:LoaderCore, all:Bool):Void {
		...
		if (all) {
			...
			for (p in Reflect.fields(_listenerTypes)) {
				if (p != "onProgress" && p != "onInit") {
					loader.removeEventListener(Reflect.field(_listenerTypes, p), _passThroughEvent);
				}
			}
		}
	}
	...
	public function getLoader(nameOrURL:String):Dynamic {
		...
		while (--i > -1) {
			loader = _loaders[i];
			if (loader.name == nameOrURL
				|| (Std.isOfType(loader, LoaderItem) && (try cast(loader, LoaderItem) catch (e:Dynamic) null).url == nameOrURL)) {
				return loader;
			} else if (loader.exists("getLoader")) {
	...
	private function _loadNext(event:Event = null):Void {
		...
		if (_status == LoaderStatus.LOADING) {
			var audit:Bool = ((Lambda.has(this.vars, "auditSize"))) ? cast(this.vars.auditSize, Bool) : LoaderMax.defaultAuditSize;
			...
			var loader:LoaderCore;
			var loaders:Array<Dynamic> = _loaders.copy();
			var l:Int = loaders.length;
			var activeCount:Int = 0; // use _loaders.concat() because in some rare situations, a loader's COMPLETE event might occur immediately and if autoDispose is true, the length of the array could change mid-loop causing a skip.
			_calculateProgress();
			for (i in 0...l) {
				loader = loaders[i];
				if (!this.skipPaused && loader.status == LoaderStatus.PAUSED) {
					super._failHandler(new LoaderEvent(LoaderEvent.FAIL, this,
						"Did not complete LoaderMax because skipPaused was false and " + Std.string(loader) + " was paused."),
						false);
					return;
				} else if (!this.skipFailed && loader.status == LoaderStatus.FAILED) {
					super._failHandler(new LoaderEvent(LoaderEvent.FAIL, this,
						"Did not complete LoaderMax because skipFailed was false and " + Std.string(loader) + " failed."),
						false);
					return;
	...
	override private function _progressHandler(event:Event):Void {
		...
		if (_dispatchProgress && _status != LoaderStatus.DISPOSED) {
			...
			_calculateProgress();
			if (bl == 0 && _cachedBytesLoaded == 0) { // do nothing
			} else if ((_cachedBytesLoaded != _cachedBytesTotal || _status != LoaderStatus.LOADING)
	 ...
	 */
	public static function activate(loaderClasses:Array<Dynamic>):Void { // no need to do anything - we just want to force the classes to get compiled in the swf. Each one calls the _activateClass() method in LoaderCore on its own.
	}
	...
	override private function _passThroughEvent(event:Event):Void {
		...
		if (!this.skipFailed && (event.type == "fail" || event.type == "childFail") && this.status == LoaderStatus.LOADING) {
			super._failHandler(new LoaderEvent(LoaderEvent.FAIL, this,
				"Did not complete LoaderMax because skipFailed was false and " + Std.string(event.target) + " failed."),
				false);
		}
	}
	...
	/** @inheritDoc **/
	override private function get_status():Int // if the status of children changed after the LoaderMax completed, we need to make adjustments to the LoaderMax's status.
	{
		if (_status == LoaderStatus.COMPLETED) {
			var statusCounts:Array<Dynamic> = [0, 0, 0, 0, 0, 0]; // store the counts of each type of status (index 0 is for READY, 1 is LOADING, 2 is COMPLETE, etc.
			var i:Int = _loaders.length;
			while (--i > -1) {
	...
	override private function get_auditedSize():Bool {
		...
		while (--i > -1) {
			if (!cast((_loaders[i]), LoaderCore).auditedSize
				&& cast((_loaders[i]), LoaderCore).status <= maxStatus && _loaders[i].vars.auditSize != false) {
				return false;
			}
		}
		return true;
