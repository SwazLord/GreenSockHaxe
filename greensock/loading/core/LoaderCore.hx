...
class LoaderCore extends EventDispatcher {
	...
	/** @private **/
	private static var _rootLookup:Dictionary<Dynamic, LoaderMax> = new Dictionary<Dynamic, LoaderMax>();

	/** @private **/
	private static var _isLocal:Bool = false;

	/** @private **/
	public static var _globalRootLoader:LoaderMax;

	/** @private **/
	public static var _listenerTypes:Dynamic = {
		onOpen: "open",
		onInit: "init",
		onComplete: "complete",
		onProgress: "progress",
	...
	/** @private **/
	public static var _extensions:Dynamic = {};
	...
	public function new(vars:Dynamic = null) {
		...
		_rootLoader = ((Std.isOfType(this.vars.requireWithRoot, DisplayObject))) ? _rootLookup[this.vars.requireWithRoot] : _globalRootLoader;
		...
		for (p in Reflect.fields(_listenerTypes)) {
			if (Lambda.has(this.vars, p) && Std.isOfType(this.vars[p], Function)) {
				this.addEventListener(Reflect.field(_listenerTypes, p), this.vars[p], false, 0, true);
			}
		}
	...
	private function _load():Void { // override in subclasses
	}
	...
	**/
	...
	/** @private **/
	private function _calculateProgress():Void { // override in subclasses if necessary
	}
	...
	**/
	public function auditSize():Void { // override in subclasses
	}
	...
	override public function toString():String {
		return _type
			...
			+ this.name
			+ "'"
			+ (((Std.isOfType(this, LoaderItem))) ? " (" + (try cast(this, LoaderItem) catch (e:Dynamic) null).url + ")" : "");
	}

	//---- STATIC METHODS ------------------------------------------------------------------------------------

	/** @private **/
	public static function _activateClass(type:String, loaderClass:Class<Dynamic>, extensions:String):Bool {
		if (type != "") {
	...
	private function _progressHandler(event:Event):Void {
		if (Std.isOfType(event, ProgressEvent)) {
			_cachedBytesLoaded = Std.int(cast(event, ProgressEvent).bytesLoaded);
			_cachedBytesTotal = Std.int(cast(event, ProgressEvent).bytesTotal);
			if (!_auditedSize) {
				_auditedSize = true;
