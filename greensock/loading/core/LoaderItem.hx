...
class LoaderItem extends LoaderCore {
	...
	/** @private **/
	private var _scriptAccessDenied:Bool = false;
	...
	private var _preferEstimatedBytesInAudit:Bool = false;
	...
	private var _skipAlternateURL:Bool = false;
	...
	public function new(urlOrRequest:Dynamic, vars:Dynamic = null) {
		super(vars);
		_request = ((Std.isOfType(urlOrRequest, URLRequest))) ? try
			cast(urlOrRequest, URLRequest)
		catch (e:Dynamic)
			null : new URLRequest(urlOrRequest);
		_url = _request.url;
	...
	private function _setRequestURL(request:URLRequest, url:String, extraParams:String = ""):Void {
		...
		if (extraParams != "") {
			var data:URLVariables = new URLVariables((((Std.isOfType(request.data, URLVariables))) ? Std.string(request.data) : null));
			a = extraParams.split("&");
			i = a.length;
	...
	override private function _failHandler(event:Event, dispatchError:Bool = true):Void {
			...
			// don't do (_url != vars.alternateURL) because the audit could have changed it already - that's the whole purpose of _skipAlternateURL.
		{
			...
			_url = "temp" + (Date.now()
				.getTime()); // in case the audit already changed the _url to vars.alternateURL, we temporarily make it something different in order to force the refresh in the url setter which skips running the code if the url is set to the same value as it previously was. Don't use Math.random() because for some reason, Google Display Network disallows it (citing security reasons).
