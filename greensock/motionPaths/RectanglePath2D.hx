...
class RectanglePath2D extends MotionPath
{
    ...
    override public function update(event : Event = null) : Void
    {
        ...
        while (f!= null)
        {
            ...
            if (f.autoRotate)
            {
                f.target.rotation = Math.atan2(xFactor * b + yFactor * d, xFactor * a + yFactor * c) * MotionPath._RAD2DEG + f.rotationOffset;
            }
    ...
    override public function renderObjectAt(target : Dynamic, progress : Float, autoRotate : Bool = false, rotationOffset : Float = 0) : Void
    {
        ...
        if (autoRotate)
        {
            target.rotation = Math.atan2(xFactor * m.b + yFactor * m.d, xFactor * m.a + yFactor * m.c) * MotionPath._RAD2DEG + rotationOffset;
        }
    }
