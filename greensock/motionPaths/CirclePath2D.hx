...
class CirclePath2D extends MotionPath
{
    ...
    override public function update(event : Event = null) : Void
    {
        ...
        while (f != null)
        {
            ...
            if (f.autoRotate)
            {
                ...
                px = Math.cos(angle) * _radius;
                py = Math.sin(angle) * _radius;
                f.target.rotation = Math.atan2(px * m.b + py * m.d, px * m.a + py * m.c) * MotionPath._RAD2DEG + f.rotationOffset;
            }
    ...
    override public function renderObjectAt(target : Dynamic, progress : Float, autoRotate : Bool = false, rotationOffset : Float = 0) : Void
    {
        ...
        if (autoRotate)
        {
            ...
            px = Math.cos(angle) * _radius;
            py = Math.sin(angle) * _radius;
            target.rotation = Math.atan2(px * m.b + py * m.d, px * m.a + py * m.c) * MotionPath._RAD2DEG + rotationOffset;
        }
    }
