...
class BlitMask extends Sprite
{
    ...
    public function new(target : DisplayObject, x : Float = 0, y : Float = 0, width : Float = 100, height : Float = 100, smoothing : Bool = false, autoUpdate : Bool = false, fillColor : Int = 0, wrap : Bool = false)
    {
        super();
        if (width < 0 || height < 0)
        {
