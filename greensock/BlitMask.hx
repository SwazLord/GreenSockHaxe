...
class BlitMask extends Sprite {
	...
	/** @private **/
	private static var _emptyArray:Array<BitmapFilter> = [];

	/** @private **/
	private static var _colorTransform:ColorTransform = new ColorTransform();

	/** @private **/
	private static var _mouseEvents:Array<Dynamic> = [
		MouseEvent.CLICK,
	...
	private var forceRecaptureBitmap:Bool = false;
	 ...
	 */
	public function new(target:DisplayObject, x:Float = 0, y:Float = 0, width:Float = 100, height:Float = 100, smoothing:Bool = false,
			autoUpdate:Bool = false, fillColor:Int = 0x00000000, wrap:Bool = false) {
		super();
		if (width < 0 || height < 0) {
		...
		if (_smoothing) {
			...
			super.x = Std.int((x < 0) ? x - 0.5 : x + 0.5);
			super.y = Std.int((y < 0) ? y - 0.5 : y + 0.5);
		}
		_clipRect = new Rectangle(0, 0, _gridSize + 1, _gridSize + 1);
		_bd = new BitmapData(Std.int(width + 1), Std.int(height + 1), true, _fillColor);
		_bitmapMode = true;
		this.target = target;
	}
	...
	private function _captureTargetBitmap():Void {
		...
		var prevFilters:Array<BitmapFilter> = _target.filters;
		...
		if (!_smoothing) {
			xOffset = Std.int(xOffset + 0.5);
			yOffset = Std.int(yOffset + 0.5);
		}
		...
		for (row in 0..._rows) {
			...
			for (column in 0..._columns) {
				...
				_grid[row][column] = bd = new BitmapData(Std.int(w + 1), Std.int(h + 1), true, _fillColor);
	...
	/** @private  **/
	private function _render(xOffset:Float = 0, yOffset:Float = 0, clear:Bool = true,
			limitRecursion:Bool = false):Void // note: the code in this method was optimized for speed rather than readability or succinctness (since the whole point of this class is to help things perform better)
	{
		if (clear) {
		...
		if (_bounds.width == 0
			|| _bounds.height == 0
			|| (_wrap && (wrapWidth == 0 || wrapHeight == 0))
			|| (!_wrap && (x + _width < _bounds.x || y + _height < _bounds.y || x > _bounds.right || y > _bounds.bottom))) {
			g.clear();
		...
		if (y <= _bounds.y) {
			_destPoint.y = Std.int(_bounds.y - y);
			_sliceRect.y = -1;
		} else {
			_destPoint.y = 0;
		...
		if (x <= _bounds.x) {
			_destPoint.x = Std.int(_bounds.x - x);
			_sliceRect.x = -1;
		} else {
			_destPoint.x = 0;
	...
	**/
	...
	private function _mouseEventPassthrough(event:Event):Void {
		if (this.mouseEnabled
			&& (!_bitmapMode
				|| (Std.isOfType(event, MouseEvent)
					&& this.hitTestPoint(cast((event), MouseEvent).stageX, cast((event), MouseEvent).stageY, false)))) {
			dispatchEvent(event);
		}
	}
