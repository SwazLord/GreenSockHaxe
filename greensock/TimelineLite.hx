...
class TimelineLite extends SimpleTimeline {
	...
	public function new(vars:Dynamic = null) {
		...
		for (p in Reflect.fields(this.vars)) {
			val = Reflect.field(this.vars, p);
			if (Std.isOfType(val, Array)) {
				if (val.join("").indexOf("{self}") != -1) {
					// this.vars[p] = _swapSelfInParams(try cast(val, Array<Dynamic>) catch(e:Dynamic) null);
					Reflect.setField(this.vars, p, _swapSelfInParams(try cast(val, Array<Dynamic>) catch (e:Dynamic) null));
				}
			}
		}
	...
	public function fromTo(target:Dynamic, duration:Float, fromVars:Dynamic, toVars:Dynamic, position:Dynamic = "+=0"):Dynamic {
		return ((duration != 0 && !Math.isNaN(duration))) ? add(TweenLite.fromTo(target, duration, fromVars, toVars),
			position) : this.set(target, toVars, position);
	}
	...
	/**
		...
		* @see #staggerFromTo()
	 */
	public function staggerTo(targets:Array<Dynamic>, duration:Float, vars:Dynamic, stagger:Float, position:Dynamic = "+=0", onCompleteAll:Function = null,
			onCompleteAllParams:Array<Dynamic> = null):Dynamic {
	...
	/**
		...
		* @see #staggerFromTo()
	 */
	public function staggerFrom(targets:Array<Dynamic>, duration:Float, vars:Dynamic, stagger:Float = 0, position:Dynamic = "+=0",
			onCompleteAll:Function = null, onCompleteAllParams:Array<Dynamic> = null):Dynamic {
		vars = _prepVars(vars);
	...
	/**
		...
		* @see #staggerFrom()
	 */
	public function staggerFromTo(targets:Array<Dynamic>, duration:Float, fromVars:Dynamic, toVars:Dynamic, stagger:Float = 0, position:Dynamic = "+=0",
			onCompleteAll:Function = null, onCompleteAllParams:Array<Dynamic> = null):Dynamic {
	...
	/**
		...
		tl.call(func, ["param1"], "myLabel+=2");  //places it 2 seconds after "myLabel"
				 ...
				 * @see #remove()
	 */
	public function call(callback:Function, params:Array<Dynamic> = null, position:Dynamic = "+=0"):Dynamic {
	...
	/**
		...
		timeline.addPause(4, yourFunction, ["param1", "param2"]);
				 ...
				 * @see #call()
	 */
	public function addPause(position:Dynamic = "+=0", callback:Function = null, params:Array<Dynamic> = null):Dynamic {
		return call(_pauseCallback, ["{self}", callback, params], position);
	}

	/** @private **/
	private function _pauseCallback(tween:TweenLite, callback:Function = null, params:Array<Dynamic> = null):Void {
	...
	/** @private **/
	private static function _prepVars(vars:Dynamic):Dynamic // to accommodate TweenLiteVars and TweenMaxVars instances for strong data typing and code hinting
	{
	...
	public static function exportRoot(vars:Dynamic = null, omitDelayedCalls:Bool = true):TimelineLite {
		...
		while (tween != null) {
			next = tween._next;
			if (!omitDelayedCalls || !(Std.isOfType(tween, TweenLite) && cast((tween), TweenLite).target == tween.vars.onComplete)) {
	...
	override public function add(value:Dynamic, position:Dynamic = "+=0", align:String = "normal", stagger:Float = 0):Dynamic {
		...
		if (!(Std.isOfType(value, Animation))) {
			if (Std.isOfType(value, Array)) {
				...
				for (i in 0...l) {
					...
					add(child, curTime);
					if (as3hx.Compat.typeof((child)) == "string" || as3hx.Compat.typeof((child)) == "function") { // do nothing
	...
	private function _parseTimeOrLabel(timeOrLabel:Dynamic, offsetOrLabel:Dynamic = 0, appendIfAbsent:Bool = false, ignore:Dynamic = null):Float {
		...
		if (Std.isOfType(ignore, Animation) && ignore.timeline == this) {
			remove(ignore);
		} else if (Std.isOfType(ignore, Array)) {
			i = ignore.length;
			while (--i > -1) {
				if (Std.isOfType(Reflect.field(ignore, Std.string(i)), Animation)
					&& Reflect.field(ignore, Std.string(i)).timeline == this) {
					remove(Reflect.field(ignore, Std.string(i)));
				}
			}
		}
		if (as3hx.Compat.typeof((offsetOrLabel)) == "string") {
			return _parseTimeOrLabel(offsetOrLabel,
				((appendIfAbsent
					&& as3hx.Compat.typeof((timeOrLabel)) == "number"
					&& !(Lambda.has(_labels, offsetOrLabel)))) ? timeOrLabel - duration() : 0,
				appendIfAbsent);
		}
			...
			// if the string is a number like "1", check to see if there's a label with that name, otherwise interpret it as a number (absolute value).
		{
			...
			if (i == -1) {
				if (!(Lambda.has(_labels, timeOrLabel))) {
					...
					if (appendIfAbsent) {
						Reflect.setField(_labels, timeOrLabel, duration() + offsetOrLabel);
						return Reflect.field(_labels, timeOrLabel);
					} else {
			...
			// offsetOrLabel = Compat.parseInt((timeOrLabel.charAt(i-1) + "1"), 10) * Number(timeOrLabel.substr(i+1));
			offsetOrLabel = Std.parseInt(timeOrLabel.charAt(i - 1) + "1") * Std.parseFloat(timeOrLabel.substr(i + 1));
			timeOrLabel = ((i > 1)) ? _parseTimeOrLabel(timeOrLabel.substr(0, i - 1), 0, appendIfAbsent) : duration();
		} else if (timeOrLabel == null) {
			timeOrLabel = duration();
		}
		return Compat.parseFloat(timeOrLabel) + offsetOrLabel;
	}
	...
	override public function render(time:Float, suppressEvents:Bool = false, force:Bool = false):Void {
		...
		if (time >= totalDur) {
			...
			_rawPrevTime = ((_duration != 0 || !suppressEvents || time != 0 || _rawPrevTime == time)) ? time : Animation._tinyNum; // when the playhead arrives at EXACTLY time 0 (right on top) of a zero-duration timeline or tween, we need to discern if events are suppressed so that when the playhead moves again (next time), it'll trigger the callback. If events are NOT suppressed, obviously the callback would be triggered in this render. Basically, the callback should fire either when the playhead ARRIVES or LEAVES this exact spot, not both. Imagine doing a timeline.seek(0) and there's a callback that sits at 0. Since events are suppressed on that seek() by default, nothing will fire, but when the playhead moves off of that position, the callback should fire. This behavior is what people intuitively expect. We set the _rawPrevTime to be a precise tiny number to indicate this scenario rather than using another property/variable which would increase memory usage. This technique is less readable, but more efficient.
			...
			// to work around occasional floating point math artifacts, round super small values to 0.
		{
			_totalTime = _time = 0;
			if (prevTime != 0
				|| (_duration == 0 && _rawPrevTime != Animation._tinyNum && (_rawPrevTime > 0 || (time < 0 && _rawPrevTime >= 0)))) {
				callback = "onReverseComplete";
			...
			if (time < 0) {
				...
				_rawPrevTime = (((_duration != 0 && !Math.isNaN(_duration))
					|| !suppressEvents
					|| time != 0
					|| _rawPrevTime == time)) ? time : Animation._tinyNum; // when the playhead arrives at EXACTLY time 0 (right on top) of a zero-duration timeline or tween, we need to discern if events are suppressed so that when the playhead moves again (next time), it'll trigger the callback. If events are NOT suppressed, obviously the callback would be triggered in this render. Basically, the callback should fire either when the playhead ARRIVES or LEAVES this exact spot, not both. Imagine doing a timeline.seek(0) and there's a callback that sits at 0. Since events are suppressed on that seek() by default, nothing will fire, but when the playhead moves off of that position, the callback should fire. This behavior is what people intuitively expect. We set the _rawPrevTime to be a precise tiny number to indicate this scenario rather than using another property/variable which would increase memory usage. This technique is less readable, but more efficient.
		...
		if (_onUpdate != null) {
			if (!suppressEvents) {
				// _onUpdate.call(null, vars.onUpdateParams);
	...
	public function _hasPausedChild():Bool {
		...
		while (tween != null) {
			if (tween._paused || ((Std.isOfType(tween, TimelineLite)) && cast((tween), TimelineLite)._hasPausedChild())) {
				return true;
			}
			tween = tween._next;
		}
	...
	public function getChildren(nested:Bool = true, tweens:Bool = true, timelines:Bool = true, ignoreBeforeTime:Float = -9999999999):Array<Dynamic> {
		...
		while (tween != null) {
			...
			} else if (Std.isOfType(tween, TweenLite)) {
				if (tweens) {
					a[cnt++] = tween;
				}
			} else {
		...
		if (adjustLabels) {
			for (p in Reflect.fields(_labels)) {
				if (Reflect.field(_labels, p) >= ignoreBeforeTime) {
					Reflect.setField(_labels, p, Reflect.field(_labels, p) + amount);
				}
			}
		}
		_uncache(true);
	...
	override public function totalDuration(?value:Float):Dynamic {
		if (value == null) {
			if (_dirty) {
				...
				var prevStart:Float = Math.POSITIVE_INFINITY;
				var prev:Animation;
				var end:Float;
				while (tween != null) {
